...
export const metadata = {
  ...
  description:
    "Apply to join Barracks Media Network — a selective network for podcasters focused on helping others and meaningful storytelling.",
};
...
export default function ApplyPage() {
  ...
  async function submit(e) {
    ...
    if (!canSubmit) {
      setError("Please complete all required fields (and write a bit more in the final answer).");
      return;
    }
      ...
      <div className="mx-auto max-w-5xl px-5 py-16">
        ...
        <section className="rounded-3xl border border-white/10 bg-white/[0.04] backdrop-blur p-10">
          ...
          <p className="mt-5 text-lg text-white/80 max-w-3xl leading-relaxed">
            We’re selective on purpose. We accept podcasts that do{" "}
            <span className="font-semibold text-white">one or both</span>:
            <br />
        ...
        <section className="mt-10 rounded-3xl border border-white/10 bg-white/[0.04] p-10">
          ...
          <form onSubmit={submit} className="mt-8 grid gap-5">
            ...
            <div className="grid md:grid-cols-3 gap-5">
              <Field label="Publishing frequency" required>
                <select
                  ...
                  onChange={(e) =>
                    setForm({ ...form, publish_frequency: e.target.value })
                  }
                  className="w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-white outline-none focus:ring-2 focus:ring-white/20"
                >
            ...
            <Field label="If you choose Partner, how will you provide stats proof? (optional)">
              <input
                value={form.stats_proof_url}
                onChange={(e) => setForm({ ...form, stats_proof_url: e.target.value })}
                className="w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-white outline-none focus:ring-2 focus:ring-white/20"
                placeholder="Link to Google Drive folder, Dropbox folder, etc."
              />
              <p className="mt-2 text-xs text-white/55">
                Tip: If we put file uploads directly in a form, it forces login. A shared folder link keeps this simple.
              </p>
            </Field>
            ...
            <Field label="Why do you want to join Barracks Media Network?" required>
              <textarea
                ...
                placeholder="Tell us what your show stands for, who it serves, and why this network is a fit (minimum a couple sentences)."
              />
              <p className="mt-2 text-xs text-white/55">
                We’re looking for alignment and intent. “I want downloads” won’t pass review by itself.
              </p>
            </Field>
